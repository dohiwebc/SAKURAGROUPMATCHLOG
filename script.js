...
const GUEST_PLAYER_SLOTS = [
  { id: "guest-1", name: "助っ人" },
  { id: "guest-2", name: "助っ人2" },
  { id: "guest-3", name: "助っ人3" },
  { id: "guest-4", name: "助っ人4" },
];
...
function renderCheckMode() {
  ...
  if (state.selectedTournamentId && state.selectedMatchId) {
    ...
    const unenteredPlayers = tournamentEligiblePlayers
...
function getRosterStartersAndBench() {
  ...
  const starterIdsOrdered = Array.isArray(match?.startingMemberIds)
    ? match.startingMemberIds.filter((id) => getTournamentEligiblePlayers().some((p) => p.id === id))
    : [];
  const eligiblePlayers = getTournamentEligiblePlayers();
...
function seedLineupDraftFromMatchAndRecords() {
  const match = getSelectedMatchNormalized();
  const activeIds = new Set(getTournamentEligiblePlayers().map((player) => player.id));
  ...
  const fromRecords = getTournamentEligiblePlayers()
