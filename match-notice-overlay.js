...
(function () {
  ...
  function drawTrendChart(container, tooltipEl, trend, selectedId, order = "asc", stageFilter = "all") {
    ...
    svg.append("path").datum(points).attr("fill", "none").attr("stroke", "#84c3ff").attr("stroke-width", 2).attr("d", mkLine("catchRate"));
    svg.append("path").datum(points).attr("fill", "none").attr("stroke", "#ffd166").attr("stroke-width", 2).attr("d", mkLine("attackRate"));
  ...
  function ensureDom() {
    ...
    const close = () => {
      ...
      if (shouldReturn && typeof window.__siteNavOpenNotices === "function") {
        window.setTimeout(() => {
          void window.__siteNavOpenNotices();
        }, 0);
      }
    };
